...
const handleExportPDF = async () => {
  setExportMode(true); // Show all content
  ...
  setExportMode(false); // Restore UI
};
  /* ------------------- data fetch ------------------- */
  useEffect(() => {
    if (userId) fetchData();
  ...
  useEffect(() => {
    if (polling && selectedSurvey && userId) {
      const id = setInterval(() => {
        fetchData();
        const survey = surveyHistory.find(
          (s) => s.upload_id === selectedSurvey
        );
  ...
  const currentSurvey = surveyHistory.find(
    (s) => s.upload_id === selectedSurvey
  );
