...
def lambda_handler(event, context):
    ...
    try:
        ...
        if user_id:
            ...
            return { 'statusCode': 200, 'headers': cors_headers(event), 'body': json.dumps({'surveys': convert_decimal(items), 'upload_count': len(items)}) }
        else:
            # Scan all surveys (not recommended for large tables)
            resp = table.scan()
            items = resp.get('Items', [])
            return { 'statusCode': 200, 'headers': cors_headers(event), 'body': json.dumps({'surveys': convert_decimal(items)}) }
